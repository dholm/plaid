...
def _create_patch(EmailMixin, db):
    class Patch(EmailMixin, db.Model):
        ...
        ancestor = db.relationship('Patch', backref="successor",
                                   remote_side=[id])
        ...
        def mbox(self):
            ...
            mbox['From'] = '%s <%s>' % (self.submitter.name,
...
def _create_comment(EmailMixin, db):
...
def _create_topic(db):
    topics = db.Table('topics',
                      db.Column('patch_id', db.Integer,
                                db.ForeignKey('patch.id')),
                      db.Column('topic.id', db.Integer,
...
class Schema(object):
    def __init__(self, db):
        ...
        self.Submitter = _create_submitter(db)
        self.Project = _create_project(db)
        self.EmailMixin = _create_email_mixin(db)
        self.Patch = _create_patch(self.EmailMixin, db)
        self.Comment = _create_comment(self.EmailMixin, db)
        self.Series = _create_series(db)
        self.Topic = _create_topic(db)
        self.Tag = _create_tag(db)
